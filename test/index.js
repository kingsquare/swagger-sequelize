...
if (!process.env.DATABASE_URI) {
  console.log(
    "Please set the environment variable DATABASE_URI to test against. (MariaDB)"
  );
  process.exit(1);
}
...
var swaggerSpec = JSON.parse(
  fs.readFileSync(__dirname + "/fixtures/swagger.json", "utf-8")
);
...
exports.testSomething = test => {
  ...
  Object.keys(swaggerSpec.definitions).forEach(modelName => {
    models[modelName] = sequelize.define(
      modelName,
      swaggerSequelize.generate(swaggerSpec.definitions[modelName]),
